...
public class ChessGUI {
	...
	public final void initializeGui(){
		...
		Action surrenderGameAction = new AbstractAction("Surrender Game") {
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				//playerLabel.setBorder(new CompoundBorder(new LineBorder(Color.CYAN,5), new BevelBorder(BevelBorder.RAISED)));
		...
		JPanel playerStatsPanel = new JPanel(new GridLayout(4,0)); //JPanel to hold the chessBoard JPanel
		//playerStatsPanel.add(playerStats);
		//playerStatsPanel.add(playerLabel);
					...
					public void actionPerformed(ActionEvent e){
						if(control.checkIfLegal()){ //enters loop only when checkIfLegal determines the last two clicked squares add up to a legal move
							//if(!model.getCheck()){//;
								...
								move();
								//model.setCheck();
								model.setTurnCounter(model.getTurnCounter()+1); //essentially changes player turn
						...
						/*if(model.getCheck()){
							...
							playerStats.add(checkLabel); //notifies player of check status
							//model.checkBehavior();
						}else{
							System.out.println("Check is Not Set.");
							checkLabel.setText("Check Not True");
							checkLabel.setVisible(false);
							//playerStats.remove(checkLabel);
						}
						*/
	...
	private String surrenderGame(){
		...
		if(control.playerTurn()){
			...
			whoSurrenders = control.whoseTurn();// - model.lastPlayed; //0 is black, 1 is white, lastPlayed is whoever made the last legal move. This determines who clicked surrender
		}
		clearBoard();
