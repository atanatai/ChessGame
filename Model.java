...
public class Model {
	...
	int lastPlayed = 0; //keeps track of which player made the last legal move, used to track available undo moves. 1=white, 0=black
	
	private boolean check = false;
	private boolean checkMate = false;
	int[] threatPosition = new int[3]; //records where the check/check mate is coming from
	
	private Controller control = new Controller();
	
	public void setCheck(){
		//int recentClicks[] = {lastClicked[0], lastClicked[1], justClicked[0], justClicked[1]}; // The lastClicked values are what is going to be moved into the justClicked values
		//check = false; //this function currently does not set check to false, an attempted move that will block the check will do that
		int checkCounter = 0;
		if(control.playerTurn()){ //if it is player 1 (white)'s turn
			int[] kingsPosition = getBlackKingLocation();
			for(int i = 0; i < occupiedSquares.length; i++){
				for(int j = 0; j <occupiedSquares.length; j++){
					if(occupiedSquares[i][j] != 10 && colorOfSquares[i][j] == 1){ //if occupied square has white piece
						...
						//lastClicked[0] = i;
						//lastClicked[1] = j;
						//justClicked[0] = kingsPosition[0];
						//justClicked[1] = kingsPosition[1];
					if(control.checkIfLegalForCheck(i, j, kingsPosition[0], kingsPosition[1], occupiedSquares[i][j], occupiedSquares, colorOfSquares)){
						check = true;
						checkCounter ++;
						threatPosition[0] = i;
						threatPosition[1] = j;
		...
		}else{ //it is player black's turn (player 2)
			int[] kingsPosition = getWhiteKingLocation();
			for( int i = 0; i < occupiedSquares.length; i++){
				for(int j = 0; j <occupiedSquares.length; j++){
					if(occupiedSquares[i][j] != 10 && colorOfSquares[i][j] == 0){ //if occupied square has black piece
						System.out.println("ColorOfSquares: "+colorOfSquares[i][j]+" and Last played: "+lastPlayed);
						System.out.println("Found a possible "+colorLookup(lastPlayed)+" threat at: ["+i+","+j+"]");
						//lastClicked[0] = i;
						//lastClicked[1] = j;
	...
	protected int[] getWhiteKingLocation(){
		int[] kingLocation = new int[3];
			for(int i = 0; i < occupiedSquares.length; i++){
				for(int j = 0; j <occupiedSquares.length; j++){
					if(occupiedSquares[i][j] == 0 && colorOfSquares[i][j] == 1){ //if piece is a white king
						System.out.println("Found the white king at: ["+i+","+j+"]");
						kingLocation[0]=i;
						kingLocation[1]=j;
						kingLocation[2]=1; //third index holds king's color
					}
				}
			}
		return kingLocation;
	...
	protected int[] getBlackKingLocation(){
		int[] kingLocation = new int[3];
			for(int i = 0; i < occupiedSquares.length; i++){
				for(int j = 0; j <occupiedSquares.length; j++){
					if(occupiedSquares[i][j] == 0 && colorOfSquares[i][j] == 0){ //if piece is a black king
						...
						kingLocation[0]=i;
						kingLocation[1]=j;
						kingLocation[2]=0; //third index holds king's color
					}
				}
			}
		return kingLocation;
	...
	public void checkBehavior(){
				...
				//if(control.playerTurn()){ //if it is player 1 (white)'s turn
					...
					for(int i = 0; i < defaultOccupied.length; i++){
						for(int j = 0; j <defaultOccupied.length; j++){
							if(defaultOccupied[i][j] != 10 && defaultColored[i][j] == 0){ //if occupied square has black piece
								//System.out.println("ColorOfSquares: "+colorOfSquares[i][j]+" and Last played: "+lastPlayed);
								//System.out.println("Found a possible "+colorLookup(lastPlayed)+" threat at: ["+i+","+j+"]");
								if(control.checkIfLegalForCheck(i, j, kingsPosition[0], kingsPosition[1], defaultOccupied[i][j], defaultOccupied, defaultColored)){
									check = true;
									checkCounter ++;
									threatPosition[0] = i;
									threatPosition[1] = j;
					...
					//}
				//}else{ //it is player black's turn (player 2)
					kingsPosition = getBlackKingLocation();
					for(  i = 0; i < defaultOccupied.length; i++){
						for(int j = 0; j <defaultOccupied.length; j++){
							if(defaultOccupied[i][j] != 10 && defaultColored[i][j] == 1){ //if occupied square has white piece
								//System.out.println("ColorOfSquares: "+colorOfSquares[i][j]+" and Last played: "+lastPlayed);
								//System.out.println("Found a possible "+colorLookup(lastPlayed)+" threat at: ["+i+","+j+"]");
								if(control.checkIfLegalForCheck(i, j, kingsPosition[0], kingsPosition[1], defaultOccupied[i][j], defaultOccupied, defaultColored)){
									check = true;
		...
		//occupiedSquares = defaultOccupied;
		//colorOfSquares = defaultColored;
	}
